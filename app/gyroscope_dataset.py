...
METADATA_PREFIX = "# gyroscope_metadata="
...
class GyroscopeSampleRecord:
    ...
    @classmethod
    def from_csv_row(cls, row: dict[str, str]) -> GyroscopeSampleRecord:
...
class GyroscopeDataset:
    def __init__(
        self,
        name: str,
        *,
        records: list[GyroscopeSampleRecord] | None = None,
    ...
    def concatenate(self, other_dataset: GyroscopeDataset, *, name: str | None = None) -> GyroscopeDataset:
        ...
        return GyroscopeDataset(merged_name, records=merged_records)
    ...
    @classmethod
    def from_csv(cls, path: str) -> GyroscopeDataset:
        if ImuDataset.is_imu_csv(path):
            return ImuDataset.from_csv(path).project_gyroscope_dataset()
        ...
        with open(path, "r", encoding="utf-8", newline="") as fh:
            ...
            records = [GyroscopeSampleRecord.from_csv_row(row) for row in reader]
